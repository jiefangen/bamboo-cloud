...
import org.panda.support.cas.core.security.feign.FeignJwtAuthenticationFilter;
...
public abstract class WebMvcSecurityConfigurerSupport extends WebSecurityConfigurerAdapter {
    ...
    public WebHttpSecurityConfigurer logoutConfigurer() {
        ...
        return http -> {
            Map<String, LogoutHandler> handlers = applicationContext.getBeansOfType(LogoutHandler.class);
            if (handlers.size() > 0) {
                LogoutConfigurer<HttpSecurity> logoutConfigurer = http.logout();
                handlers.values().forEach(logoutConfigurer::addLogoutHandler);
            }
        };
    }
